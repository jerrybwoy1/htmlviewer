...
html,body{max-width:100%;overflow-x:hidden}img,video,canvas,svg{max-width:100%;height:auto}table{max-width:100%}iframe{max-width:100%}
@media(max-width:${w}px){
  .debooger-auto-grid,[class*="grid"],[class*="cards"]{grid-template-columns:minmax(0,1fr)}
  [class*="sidebar"],[class*="sidenav"]{max-width:min(88vw,320px)}
  [class*="modal"],[role="dialog"]{max-width:calc(100vw - 24px);max-height:calc(100vh - 24px)}
