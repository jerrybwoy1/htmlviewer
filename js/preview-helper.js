  ...
  return `(()=>{
      ...
      async function shot(label,phase=''){
        try{
          ...
          const w=Math.min(Math.max(root.scrollWidth,document.body?.scrollWidth||0),2200);
          const h=Math.min(Math.max(root.scrollHeight,document.body?.scrollHeight||0),3200);
          ...
          parent.postMessage({type:'debooger-screenshot',label,phase,data,score},origin);
          return true;
        }catch(err){
          parent.postMessage({type:'debooger-screenshot-error',message:String(err?.message||err),label,phase},origin);
          return false;
        }
      }
      function controlLabel(el){
        return (el.getAttribute('aria-label')||el.innerText||el.title||el.name||el.id||el.tagName||'Control')
          .replace(/\s+/g,' ').trim().slice(0,80);
      }
      function safeControl(el){
        if(!visible(el)||el.disabled)return false;
        const label=controlLabel(el);
        const danger=/delete|remove|send|call|dial|pay|purchase|submit|publish|export|save|message|email|sms|whatsapp|logout|sign out|archive|trash|clear all|kill|stop/i;
        const useful=/menu|more|filter|column|setting|option|view|detail|info|help|sort|tab|expand|collapse|open|toggle|show|hide|sheet|page|next|previous/i;
        const isCheck=el.matches?.('input[type="checkbox"],input[type="radio"]');
        return !danger.test(label)&&(useful.test(label)||isCheck||el.hasAttribute('aria-haspopup')||el.hasAttribute('aria-expanded'));
      }
      async function restoreControl(el,beforeChecked,beforeExpanded){
        try{
      ...
      async function testControls(){
        const controls=[...document.querySelectorAll('button,[role="button"],[aria-haspopup],[aria-expanded],input[type="checkbox"],input[type="radio"]')]
          .filter(safeControl).slice(0,8);
        let tested=0;
        for(const el of controls){
          ...
          try{
            ...
            if(newErrors.length){status='FAIL';detail='JavaScript error: '+newErrors[0]}
            else if(sigAfter===sigBefore){status='WARN';detail='Click completed but no visible state change was detected'}
            else detail='Visible state changed after the click';
          ...
          tested++;
          await restoreControl(el,checkedBefore,expandedBefore);
          await sleep(120);
        }
